package edu.depaul.cdm.se452.dreamteam.windyhamhotel.config;
...
public class AppConfig {
    @Bean
	CommandLineRunner hotelRunner(HotelRepository hotelRepository, AddressRepository hotelAddressRepository, RoomRepository roomRepository) {
		return args -> {
    ...
    @Bean
    CommandLineRunner reservationRunner(ReservationRepository reservationRepository) {
        return args -> {
            Guest guest1 = new Guest("Sebastian", "G", "sg@depaul.edu", "111-222-3333");
            Guest guest2 = new Guest("Jackson", "M", "jm@depaul.edu", "333-444-5555");
            Bill bills1 = new Bill("May 15, 2021", "19.99", "299.99", "319.98");
            Bill bills2 = new Bill("May 20, 2021", "29.99", "399.99", "429.98");

            Reservation reservation1 = new Reservation(1, 1, 2, "May 15, 2021", "May 16, 2021", 1, 2, guest1, bills1);
            Reservation reservation2 = new Reservation(2, 0, 2, "May 20, 2021", "May 21, 2021", 1, 2, guest2, bills2);
	...
	@Bean
	CommandLineRunner foodRunner(DrinkRepository drinkRepository, FoodRepository foodRepository) {
		return args -> {
	...
	@Bean
	CommandLineRunner reviewRunner(ReviewRepository reviewRepository, SequenceGeneratorService sequenceGeneratorService) {
		return args -> {
	...
	@Bean
	CommandLineRunner hotelServiceRunner(HotelServiceRepository hotelServiceRepository) {
		return args -> {
	...
	@Bean
	CommandLineRunner complaintRunner(ComplaintRepository complaintRepository, SequenceGeneratorService sequenceGeneratorService) {
		return args -> {
